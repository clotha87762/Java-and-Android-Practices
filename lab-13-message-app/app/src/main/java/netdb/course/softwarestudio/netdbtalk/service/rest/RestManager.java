package netdb.course.softwarestudio.netdbtalk.service.rest;
...
import netdb.course.softwarestudio.netdbtalk.R;
import netdb.course.softwarestudio.netdbtalk.service.rest.model.Image;
import netdb.course.softwarestudio.netdbtalk.service.rest.model.PostResource;
import netdb.course.softwarestudio.netdbtalk.service.rest.model.Resource;
import netdb.course.softwarestudio.netdbtalk.service.rest.ui.RestImageView;
...
public class RestManager {
    ...
    public <T extends Resource> void listResource(Class<T> elemCls, Map<String, String> params,
        ...
        try {
            ...
            Class<T[]> arrayCls = (Class<T[]>) Class.forName("[L" + elemCls.getName() + ";");
             GsonRequest<T[]> req = new  GsonRequest<T[]>(getCollectionUrl(elemCls, parents), params, null,
    ...
    public <T extends Resource> void postResource(Class<T> cls, final T resource,
                                                                                           final PostResourceListener<T> listener,
        ...
        try {
            Log.d(TAG, "Posting resource to " + getCollectionUrl(cls, parents) + "...");
             GsonRequest<T> req = new  GsonRequest<T>(Request.Method.POST,
                    getCollectionUrl(cls, parents), null, resource, new Response.Listener< GsonResponse<T>>() {
                @Override
                public void onResponse( GsonResponse<T> gRes) {
                    Log.d(TAG, "Response: " + gRes);
    ...
    public <T extends Resource> void putResource(Class<T> cls, T resource,
                                                                                          final PutResourceListener<T> listener, String tag,
                                                                                          Resource... parents) {
        try {
            Log.d(TAG, "Putting resource to " + getResourceUrl(cls, resource.getId(), parents) + "...");
             GsonRequest<T> req = new  GsonRequest<T>(Request.Method.PUT,
                    getResourceUrl(cls, resource.getId(), parents), null, resource, new Response.Listener< GsonResponse<T>>() {
                @Override
                public void onResponse( GsonResponse<T> gRes) {
                    Log.d(TAG, "Response: " + gRes);
                    if (listener != null) listener.onResponse(gRes.getCode(), gRes.getHeaders());
    ...
    public <T extends Resource> void deleteResource(Class<T> cls, String id,
                                                                                             final DeleteResourceListener<T> listener, String tag,
                                                                                             Resource... parents) {
        try {
            Log.d(TAG, "Deleting resource from " + getResourceUrl(cls, id, parents) + "...");
             GsonRequest<T> req = new  GsonRequest<T>(Request.Method.DELETE,
                    getResourceUrl(cls, id, parents), null, null, new Response.Listener< GsonResponse<T>>() {
                @Override
                public void onResponse( GsonResponse<T> gRes) {
                    Log.d(TAG, "Response: " + gRes);
                    if (listener != null) listener.onResponse(gRes.getCode(), gRes.getHeaders());
    ...
    public <T extends Resource> String getCollectionUrl(Class<T> cls, Resource... parents) throws
        ...
        sb.insert(0, app.getString(R.string.app_rest_server_url));
        return sb.toString();
    }
    ...
    public interface PostResourceListener<T> extends ErrorListener {
        void onResponse(int code, Map<String, String> headers);
    }

    public interface PutResourceListener<T> extends ErrorListener {
        void onResponse(int code, Map<String, String> headers);
    }

    public interface DeleteResourceListener<T> extends ErrorListener {
        void onResponse(int code, Map<String, String> headers);
    }

    public interface PostImageListener<T> extends ErrorListener {
